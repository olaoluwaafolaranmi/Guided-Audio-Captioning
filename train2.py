...
decode_output, greedy_decode, AverageMeter, set_logger
...
def train(model, dataloader, optimizer, scheduler, device, epoch,criterion):

    # print(len(dataloader.dataset.keywords))
    # print(len(dataloader.dataset.captions))
    ...
    for batch_id, (audio, text, audio_names, _, keywords) in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        # text = text.to(device)
        text, text_len = model.tokenizer(text)
        text = text.to(device)
        keywords = keywords.to(device)
        # text_pad_mask = set_tgt_padding_mask(text, text_len)
        # text_pad_mask = text_pad_mask.transpose(0,1)

        y_hat = model(audio, text, keyword=keywords)
                      #, target_padding_mask=text_pad_mask)

        text = text[:, 1:]  # exclude <sos>
        y_hat = y_hat.transpose(0, 1)  # batch x words_len x ntokens
        y_hat = y_hat[:, :text.size()[1], :]  # truncate to the same length with target

        loss = criterion(y_hat.contiguous().view(-1, y_hat.size()[-1]),
                             text.contiguous().view(-1))
        # print(loss)
        loss.backward()
        optimizer.step()
...
def validate(data_loader, model, device, log_dir, epoch, beam_size):
    ...
    sos_ind = model.sos_ind
    eos_ind = model.eos_ind
    
    # print(len(data_loader.dataset.keywords))
    # print(len(data_loader.dataset.captions))
    # print(data_loader.dataset.keywords)
    ...
    with torch.no_grad():
        ...
        for i, batch_data in tqdm(enumerate(data_loader), total=len(data_loader)):
            audios, caption_dict, audio_names, audio_ids, keywords = batch_data
            # move data to GPU
            audios = audios.to(device)
            keywords = keywords.to(device)
            
            if beam_size == 1:
                output = greedy_decode(model, audios, keyword=keywords, sos_ind=sos_ind, eos_ind=eos_ind)
            else:
                output = beam_decode(audios, model, keyword=keywords, sos_ind=sos_ind, eos_ind=eos_ind, beam_width=beam_size)
        ...
        captions_pred, captions_gt = decode_output(y_hat_all, ref_captions_dict, file_names_all,word_list,
...
def main():
    ...
    if config['training']['label_smoothing']:
        criterion = LabelSmoothingLoss(model.ntoken, smoothing=0.1)
    ...
    for epoch in range(1, config["training"]["epochs"] + 1):
        ...
        train_statics = train(model, train_loader, optimizer, scheduler, device, epoch,criterion)
